...
function resetGame() {
  ...
  point.innerHTML = Math.floor(game.point);
  hp.innerHTML = Math.floor(game.hp);
  remaintime.innerHTML = Math.floor(game.time);
}
...
function addEnergy() {
  game.point += game.coinValue;
  point.innerHTML = Math.floor(game.point);
}

function removeEnergy() {
  game.hp -= game.ennemyValue;
...
function updatePlane() {
  ...
  airplane.mesh.position.x +=
    (targetX - airplane.mesh.position.x + 30) *
    deltaTime *
...
function init(event) {
  ...
  remaintime = document.getElementById('remain-time');
...
function start() {
  ...
  timer = setInterval(() => {
    game.time--;
    remaintime.innerHTML = game.time;
    if (game.time <= 0) {
      clearInterval(timer);
      stop();
    }
...
function stop() {
  clearInterval(timer);
  resetGame();
  endCb.forEach(cb => typeof cb === 'function' && cb());
}
function dead() {
  clearInterval(timer);
  game.status = 'gameover';
  deadCb.forEach(cb => typeof cb === 'function' && cb());
}

function on(type, cb) {
  type === 'end' && endCb.push(cb);
  type === 'dead' && deadCb.push(cb);
}
