...
function initScreenAnd3D() {
  ...
  camera.position.x = 0;
  camera.position.z = game.cameraPosGame;
  ...
  renderer.setClearColor(game.malusClearColor, game.malusClearAlpha);
...
function handleMouseDown(event) {
  if (game.gameStatus == 'play') hero.jump();
  else if (game.gameStatus == 'readyToReplay') {
    replay();
  }
}
...
function createFloor() {
  floorShadow = new THREE.Mesh(
    new THREE.SphereGeometry(game.floorRadius, 50, 50),
  ...
  floorGrass = new THREE.Mesh(
    new THREE.SphereGeometry(game.floorRadius - 0.5, 50, 50),
...
Hero.prototype.run = function() {
  ...
  this.runningCycle += game.delta * s * 0.7;
...
function updateMonsterPosition() {
  monster.run();
  game.monsterPosTarget -= game.delta * game.monsterAcceleration;
  game.monsterPos += (game.monsterPosTarget - game.monsterPos) * game.delta;
  if (game.monsterPos < 0.56) {
    gameOver();
  }

  var angle = Math.PI * game.monsterPos;
  monster.mesh.position.y =
    -game.floorRadius + Math.sin(angle) * (game.floorRadius + 12);
  monster.mesh.position.x = Math.cos(angle) * (game.floorRadius + 15);
...
function gameOver() {
  ...
  TweenMax.to(game, 1, { speed: 0 });
  TweenMax.to(camera.position, 3, { z: game.cameraPosGameOver, y: 60, x: -30 });
...
function replay() {
  game.gameStatus = 'preparingToReplay';
...
function createFirs() {
  ...
  for (var i = 0; i < nTrees; i++) {
    ...
    fir.mesh.position.y =
      Math.sin(theta) * Math.sin(phi) * (game.floorRadius - 10);
...
function updateCarrotPosition() {
  carrot.mesh.rotation.y += game.delta * 6;
  carrot.mesh.rotation.z = Math.PI / 2 - (game.floorRotation + carrot.angle);
  carrot.mesh.position.y =
    -game.floorRadius +
    Math.sin(game.floorRotation + carrot.angle) * (game.floorRadius + 50);
  carrot.mesh.position.x =
    Math.cos(game.floorRotation + carrot.angle) * (game.floorRadius + 50);
}
...
function updateObstaclePosition() {
  ...
  if (game.floorRotation + obstacle.angle > 2.5) {
    obstacle.angle = -game.floorRotation + Math.random() * 0.3;
  ...
  obstacle.mesh.position.y =
    -game.floorRadius +
    Math.sin(game.floorRotation + obstacle.angle) * (game.floorRadius + 3);
  obstacle.mesh.position.x =
    Math.cos(game.floorRotation + obstacle.angle) * (game.floorRadius + 3);
}

function updateFloorRotation() {
  game.floorRotation += game.delta * 0.03 * game.speed;
...
function createObstacle() {
  ...
  obstacle.mesh.position.y = game.floorRadius + 4;
  obstacle.nod();
...
function getMalus() {
  ...
  TweenMax.to(obstacle.mesh.rotation, 4, {
    ...
    onComplete: function() {
      ...
      obstacle.body.rotation.y = Math.random() * Math.PI * 2;
      obstacle.angle = -game.floorRotation - Math.random() * 0.4;
  ...
  //
  game.monsterPosTarget -= 0.04;
  TweenMax.from(game, 0.5, {
    malusClearAlpha: 0.5,
    onUpdate: function() {
      renderer.setClearColor(game.malusClearColor, game.malusClearAlpha);
    },
...
function resetGame() {
  ...
  game.monsterPos = 0.56;
  game.monsterPosTarget = 0.65;
  ...
  // audio.play();
  updateLevel();
  levelInterval = setInterval(updateLevel, game.levelUpdateFreq);
}
